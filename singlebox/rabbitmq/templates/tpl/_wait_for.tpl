...
- name: "wait-elk"
  ...
  imagePullPolicy: IfNotPresent
{{- if .Values.ha }}
  command: ['sh', '-c', 'until nc -z elasticsearch.default.svc.cluster.local {{ .Values.elastic_cluster.service.port }} -w 2; do sleep 2; done']
{{- else }}
  command: ['sh', '-c', 'until nc -z elk.default.svc.cluster.local {{ .Values.elk.service.elastic_port }} -w 2; do sleep 2; done']
{{- end }}
{{- end -}}
